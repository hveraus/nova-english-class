...
### 其他语言 = 转录失误（2026-07 确认）
- **本课程是纯英语课，师生只说英语，不存在双语教学**
- 转录中出现的任何非英语文本（葡萄牙语、中文、德语等）**一律视为转录引擎的语言误识别，直接排除**，不计入任何统计（延迟、错误、停滞、行为信号均不计）
- 成因：转录引擎把英语语音误配到其他语言模型，产出无关语言文本（如 L8 42:50 德语"Wir schreiben…"、L9 04:13 葡语 blob）
- 处理方式：在 version_note 中注明"XX:XX 处出现非英语转录文本，系转录误识别，已排除"即可，不做其他解读
